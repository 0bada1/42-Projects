...
/*   Updated: 2022/07/28 18:33:09 by ooutabac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_place_holder(va_list args, char format)
{
	...
	else if (format == 'p')
	{
		...
		counter += ft_putnbr_fd_u(va_arg(args, unsigned int), 1, 0);
	return (counter);
}
...
/*int	main(void)
{
	...
	//char	letter = 'O';
	char	*letters = NULL;
	int 	a = ft_printf("This is %s Obada\n", letters);
	int 	b = printf("This is %s Obada\n", letters);
	printf("%i\n%i\n", a, b);
}*/